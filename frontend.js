...
async function transcribeFile(filePath, model, language, responseFormat, temperature) {
    ...
    const response = await fetch(`${process.env.TRANSCRIPTION_API_BASE_URL}/audio/transcriptions`, {
        method: 'POST',
        body: formData,
    });
...
async function translateFile(filePath, model, responseFormat, temperature) {
    ...
    const response = await fetch(`${process.env.TRANSLATION_API_BASE_URL}/audio/translations`, {
        method: 'POST',
        body: formData,
    });
